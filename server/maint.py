...
INSERT_CYCLE_SQL = (
    "insert into cycles (start_time, on_duration, off_duration) values (%s,%s,%s)"
)
...
def cycle_analyze():
    with closing(conn.cursor()) as cur:
        ...
        cur.execute(
            "select time, value from raw where sensor_id = %s and time >= %s",
            (3, start),
        )
        row_count = 0
        batch = []
        for is_time, power in cur.fetchall():
            ...
            if (is_time - was_time) > max_gap:
                logging.warning(
                    "Data gap between %s and %s.  Discarding cycles.", was_time, is_time
                )
                (on_time, off_time) = (None, None)
            ...
            if not was_on and is_on:
                ...
                if on_time and off_time:
                    cycle = (
                        on_time,
                        (off_time - on_time).total_seconds(),
                        (is_time - off_time).total_seconds(),
                    )
                    logging.debug(
                        f"Inserting cycle {cycle[0].isoformat()},{cycle[1]},{cycle[2]}"
                    )
                    batch.append(cycle)
                    if len(batch) > 500:
